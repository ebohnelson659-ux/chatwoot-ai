...
<template>
  <div class="w-full">
    ...
    <div class="px-2 pb-8 list-group">
      ...
      <!-- ── AI Summary ──────────────────────────────────────────────────────
        Static accordion — intentionally not part of the draggable order
        for the MVP so we avoid touching the sidebar order store.
        Uses the same isContactSidebarItemOpen / toggleSidebarUIState
        pattern as every other accordion; state is persisted in uiSettings.
      ─────────────────────────────────────────────────────────────────────── -->
      <div class="flex flex-col gap-3 mb-3">
        <AccordionItem
          :title="$t('CONVERSATION_SIDEBAR.ACCORDION.AI_SUMMARY')"
          icon="i-lucide-sparkles"
          :is-open="isContactSidebarItemOpen('is_ai_summary_open')"
          compact
          @toggle="value => toggleSidebarUIState('is_ai_summary_open', value)"
        >
          <AiConversationSummary :conversation-id="conversationId" />
        </AccordionItem>

        <AccordionItem
          :title="$t('CONVERSATION_SIDEBAR.ACCORDION.AI_PRIORITY')"
          icon="i-lucide-bar-chart-2"
          :is-open="isContactSidebarItemOpen('is_ai_priority_open')"
          compact
          @toggle="value => toggleSidebarUIState('is_ai_priority_open', value)"
        >
          <AiPriorityScore :conversation-id="conversationId" />
        </AccordionItem>
      </div>
