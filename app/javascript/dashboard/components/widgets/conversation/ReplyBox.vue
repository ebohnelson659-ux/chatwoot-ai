...
async generateAiDraft() {
  ...
  try {
    const { data } = await axios.post(
      `/api/v1/accounts/${this.accountId}/conversations/${this.conversationId}/ai/draft_reply`
    );
    if (data.draft) {
      // Prepend the draft — preserves any existing signature already in the box.
      this.message = data.draft;
    }
  } catch (error) {
    const errorMessage =
      error?.response?.data?.error ||
      this.$t('CONVERSATION.AI_DRAFT.ERROR');
    useAlert(errorMessage);
  } finally {
    this.isGeneratingDraft = false;
  }
},
async rewriteReply(mode) {
  if (this.isRewritingReply) return;
  if (!this.message || !this.message.trim()) {
    useAlert(this.$t('AI_REPLY_REWRITER.EMPTY_DRAFT_ERROR'));
    return;
  }

  this.isRewritingReply = true;
  this.activeRewriteMode = mode;
  try {
    const { data } = await AiConversationApi.rewriteReply(
      this.conversationId,
      this.message,
      mode
    );
    ...
    useAlert(
      error?.response?.data?.error ||
        this.$t('AI_REPLY_REWRITER.ERROR')
    );
  } finally {
    this.isRewritingReply = false;
    this.activeRewriteMode = '';
  }
},
    getDraftKey(
...
<template>
  ...
  <div ref="replyEditor" class="reply-box" :class="replyBoxClass">
    ...
    <!-- ✅ AI Reply Draft button — shown only in reply mode, hidden for private notes & Copilot -->
        <div
      v-if="!isOnPrivateNote && !copilot.isActive.value && !showAudioRecorderEditor"
      class="flex items-center justify-end px-3 py-1"
    >
      <button
    ...
    <!-- ✅ AI Rewrite toolbar — shown only when there is text in the reply box -->
    <div
      v-if="!isOnPrivateNote && !copilot.isActive.value && !showAudioRecorderEditor && message && message.trim()"
      class="flex items-center gap-1 flex-wrap px-3 pb-2"
    >
      <span class="text-xs text-slate-400 dark:text-slate-500 mr-1">
        {{ $t('AI_REPLY_REWRITER.LABEL') }}
      </span>
      <button
        v-for="mode in ['professional', 'friendly', 'shorter', 'clearer']"
        :key="mode"
        ...
        :disabled="isRewritingReply"
        @click="rewriteReply(mode)"
      >
        <span v-if="isRewritingReply && activeRewriteMode === mode">
          <svg class="inline h-2.5 w-2.5 animate-spin mr-0.5" viewBox="0 0 24 24" fill="none">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" />
            <path class="opacity-75" fill="currentColor"
              d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
          </svg>
        </span>
        {{ $t(`AI_REPLY_REWRITER.MODES.${mode.toUpperCase()}`) }}
      </button>
    </div>
